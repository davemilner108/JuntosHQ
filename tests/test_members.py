...
def _create_junto(db, user):
    junto = Junto(name="Test Junto", description="For member tests", owner_id=user.id)
    db.session.add(junto)
    db.session.commit()
    return junto


def test_new_member_form(logged_in_client, db, user):
...
def test_create_member(logged_in_client, db, user):
    junto = _create_junto(db, user)
...
def test_create_member_missing_name(logged_in_client, db, user):
    junto = _create_junto(db, user)
    response = logged_in_client.post(
...
def test_edit_member_form(logged_in_client, db, user):
    junto = _create_junto(db, user)
    member = Member(name="Bob", role="Scribe", junto_id=junto.id)
    db.session.add(member)
    db.session.commit()

    response = logged_in_client.get(f"/juntos/{junto.id}/members/{member.id}/edit")
    assert response.status_code == 200
    assert b"Bob" in response.data


def test_update_member(logged_in_client, db, user):
    junto = _create_junto(db, user)
...
def test_update_member_missing_name(logged_in_client, db, user):
    junto = _create_junto(db, user)
...
def test_delete_member(logged_in_client, db, user):
    junto = _create_junto(db, user)
    member = Member(name="Gone", role="Bye", junto_id=junto.id)
    db.session.add(member)
    db.session.commit()

    response = logged_in_client.post(f"/juntos/{junto.id}/members/{member.id}/delete")
    assert response.status_code == 302
    assert db.session.execute(db.select(Member)).scalar_one_or_none() is None
